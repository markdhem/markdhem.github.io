...
if($q == 'addtocart'){
    ...
    $jim->countproducts();
}elseif($q == 'countcategory'){
    $jim->countcategory();
}elseif($q == 'countmessages'){
    $jim->countmessages();
}elseif ($q == 'messages') {
    $jim->messages(); 
}elseif ($q == 'countcustomers') {
    $jim->countcustomers(); 
}else if($q == 'checkout'){
    $jim->checkout();
}else if($q == 'verify'){
...
class Shopping {
    ...
    function countmessages(){
        $q = "select * from dbgadget.messages";
        $result = mysql_query($q);
        echo mysql_num_rows($result);
    }

    function countcustomers(){
        $q = "select * from dbgadget.order";
        $result = mysql_query($q);
        echo mysql_num_rows($result);
    }
    ...
    function messages(){
        ...
        $date = date('m/d/y h:1:s A');
        echo $q = "INSERT INTO dbgadget.messages VALUES (null, '$name','$email','$number','$message','$date') ";
        mysql_query($q);
